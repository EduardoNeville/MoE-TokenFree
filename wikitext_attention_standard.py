...
--dataset='wikitexts' \
--init_from='scratch' \
--out_dir=f'out-wikitext-byte5-num_iter-${num_iters}-exp_num_1-topk_1' \
                    ...
                    subprocess.run(
                        ...
                        env={k: str(v) for k, v in dict(
                            ...
                            **os.environ,  # Inherit original variables (e.g. conda)
                            wandb_name=f'ft_wikitext_standard_lr_{lr}_wd_{wd}_num_iter_{num_iter}_seed_{seed}_ts_{current_time}_{idx}',
                            num_iters=num_iter,
                            lr=f"{lr:0.6}",
                            min_lr=f"{min_lr:0.6}",
                            wd=f"{wd:0.6}",
