...
class CausalSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        lin_factory = get_linear_factory(to_lin_type_for("c_attn", config.lora_target_modules), config)
        self.c_attn = lin_factory(
        ...
        lin_factory = get_linear_factory(to_lin_type_for("att.c_proj", config.lora_target_modules), config)
        self.c_proj = lin_factory(
            in_features=config.n_embd,
...
class MLP(nn.Module):
    def __init__(self, config, expansion=4):
        super().__init__()
        lin_factory = get_linear_factory(to_lin_type_for("mlp.c_fc", config.lora_target_modules), config)
        self.c_fc  = lin_factory(config.n_embd, expansion * config.n_embd, bias=config.bias)
        lin_factory = get_linear_factory(to_lin_type_for("mlp.c_proj", config.lora_target_modules), config)
        self.c_proj  = lin_factory(expansion * config.n_embd, config.n_embd, bias=config.bias)
...
class Block(nn.Module):
    def __init__(self, config, mlp_block):
        super().__init__()
...
class GPTConfig:
    ...
    router_type: str = model_types.STANDARD
    topk_exp: int = 1
...
class GPT(nn.Module):
    def __init__(self, config):
        ...
        self.transformer = nn.ModuleDict(dict(
            ...
            h = nn.ModuleList([Block(config, mlp_factory(layer_id)) for layer_id in range(config.n_layer)]),
    ...
    def forward(self, input_ids, targets=None):
        ...
        if targets is not None:
            ...
            loss_for_reporting = F.cross_entropy(logits.view(-1, logits.size(-1)), targets.view(-1), ignore_index=-1)
            if self.config.load_balancing:
    ...
    def from_pretrained(cls, model_type, override_args=None):
        ...
        assert all(k == 'moe_target_modules' or k == 'topk_exp' or k == 'straight_through' or k.startswith('load_balancing') or k == 'noise_type' or k == 'dropout' or k.startswith("lora") or k.startswith("n_") or
        k == 'expert_num' or k == 'batch_size' or k == 'is_per_token' or k == 'device' or k.endswith('_type') for k in override_args)
