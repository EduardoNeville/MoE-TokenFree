...
# Base shell command
shell_cmd = r"""torchrun --nproc_per_node 1 train.py \
--dataset='wikitexts' \
--init_from='gpt2' \
                    ...
                    subprocess.run(
                        ...
                        env={k: str(v) for k, v in dict(
                            ...
                            **os.environ,  # Inherit original variables (e.g. conda)
                            wandb_name=f'ft_wikitext_moe_linear_lr_{lr}_wd_{wd}_num_iter_{num_iter}_seed_{seed}_ts_{current_time}_{idx}',
                            num_iters=num_iter,
                            lr=f"{lr:0.6}",
                            min_lr=f"{min_lr:0.6}",
                            wd=f"{wd:0.6}",
